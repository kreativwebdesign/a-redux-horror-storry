...
function* fetchPaginatedBooking({ payload: { from, to } }) {
  ...
  if (resourceValid) {
    ...
    const response = yield call(api.fetchBookings, { from, to })
...
export function* watchFetchPaginatedBooking() {
  yield takeEvery(types.FETCH.DO, fetchPaginatedBooking)
}
